...
def render_dynamic_page(page_name: str):
    ...
    raise HTTPException(status_code=404, detail=f"Page '{target_file}' not found")from fastapi import FastAPI, HTTPException, Header, Request
from fastapi.staticfiles import StaticFiles
...
# Setup Jinja2 template rendering from current working directory
...
# --- CLOUD DATABASE FUNCTIONS (VALUATION DASHBOARD - test-c.html) ---
def load_valuation_state():
    if not UPSTASH_URL or not UPSTASH_TOKEN: return None
    headers = {"Authorization": f"Bearer {UPSTASH_TOKEN}"}
    try:
...
# --- TEMPLATE ROUTING FOR HTML PAGES (DYNAMIC HEADER/FOOTER INJECTION) ---
@app.get("/", response_class=HTMLResponse)
@app.get("/index.html", response_class=HTMLResponse)
